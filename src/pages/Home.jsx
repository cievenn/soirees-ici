...
export default function Home({ navigateTo }) {
  ...
  return (
    <div className={`animate-fade-in ${THEME.bgLight} min-h-screen text-[#111] relative`}>
      ...
      {/* 1. HERO SECTION (CTA HAUT & Massive Typo) */}
      <section className="relative pt-32 pb-20 px-4 md:px-8 max-w-[1600px] mx-auto flex flex-col justify-center min-h-[95vh]">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center relative z-10">
          ...
          {/* Contenu Principal - Focus sur le CTA */}
          <div className="lg:col-span-7 flex flex-col items-start">
            
            {/* Badge */}
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-white border border-gray-200 shadow-sm mb-10 animate-fade-in-up">
            ...
            {/* Hook Massif (Police Brasika-like) */}
            <h1 className="text-[14vw] sm:text-[9vw] lg:text-[7.5rem] font-display text-[#111] leading-[0.9] tracking-normal mb-8 animate-fade-in-up drop-shadow-sm" style={{ animationDelay: '0.1s' }}>
              RÉVEILLE <br />
            ...
            {/* CTA TRÈS HAUT ET MASSIF */}
            <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-4 animate-fade-in-up z-20" style={{ animationDelay: '0.3s' }}>
              <Button onClick={() => navigateTo('agenda')} variant="primary" className="w-full sm:w-auto md:text-lg px-10 py-6 shadow-[0_20px_50px_-10px_rgba(255,0,127,0.3)]">
                <Ticket className="w-6 h-6 animate-pulse" /> SÉCURISER MA PRÉVENTE
              </Button>
              <Button onClick={() => document.getElementById('bento-grid').scrollIntoView({behavior: 'smooth'})} variant="secondary" className="w-full sm:w-auto bg-white/50 backdrop-blur-sm">
                Découvrir l'agenda
              </Button>
            </div>
          </div>

          {/* Image Hero avec effet Arche/Miroir */}
          <div className="lg:col-span-5 relative animate-fade-in-up mt-12 lg:mt-0" style={{ animationDelay: '0.4s' }}>
      ...
      {/* MARQUEE */}
      <div className="w-full bg-[#111] py-6 overflow-hidden flex items-center border-y border-white/10 shadow-2xl transform -rotate-1 scale-105 my-12 relative z-20">
        <div className="whitespace-nowrap animate-[scroll_25s_linear_infinite] flex items-center gap-12">
          {[...Array(6)].map((_, i) => (
            <React.Fragment key={i}>
              <span className="text-4xl font-display text-white uppercase tracking-wide">SOIRÉES ICI</span>
              <img src={DATA.config.logoUrl} alt="Logo" className="w-12 h-12 object-contain filter drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]" />
              <span className="text-4xl font-display text-transparent text-stroke-white uppercase tracking-wide">AGENDA 2026</span>
              <Sparkles className="w-8 h-8 text-[#ff7f00]" />
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <section id="bento-grid" className="py-24 px-4 md:px-8 max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[450px]">
          ...
          <div className={`col-span-1 md:col-span-2 lg:col-span-2 bg-white rounded-[3rem] p-10 flex flex-col justify-between relative overflow-hidden group border border-black/5 ${THEME.softShadow}`}>
            ...
            <div className="relative z-10">
              <div className="flex items-center flex-wrap gap-3 mb-6">
                <span className="px-5 py-2 bg-[#fff0f5] text-[#ff007f] rounded-full font-bold text-xs uppercase tracking-widest">{DATA.upcomingEvents[0]?.date}</span>
                <span className="px-5 py-2 bg-black text-white rounded-full font-bold text-xs uppercase tracking-widest">{DATA.upcomingEvents[0]?.location}</span>
                {DATA.upcomingEvents[0]?.tag && (
                  <span className="px-5 py-2 border border-[#ff007f] text-[#ff007f] rounded-full font-bold text-xs uppercase tracking-widest">{DATA.upcomingEvents[0]?.tag}</span>
                )}
              </div>
              <h3 className="text-5xl md:text-6xl font-display text-black leading-[1.1] max-w-lg mb-4 uppercase">
            ...
            <div className="absolute bottom-10 right-10 z-10 hidden md:block">
              <button onClick={() => navigateTo('agenda')} className="w-20 h-20 bg-black text-white rounded-full flex items-center justify-center hover:scale-105 hover:bg-gradient-to-r hover:from-[#ff007f] hover:to-[#ff7f00] transition-all duration-500 shadow-2xl group-hover:rotate-[-45deg]">
          ...
          {/* Card 2 : Photos */}
          <div className={`col-span-1 bg-[#111] rounded-[3rem] p-4 relative overflow-hidden group cursor-pointer ${THEME.softShadow}`} onClick={() => navigateTo('photos')}>
            <img src="/galeries.png" alt="Photos" className="w-full h-full object-cover rounded-[2.5rem] transition-transform duration-1000 group-hover:scale-105 opacity-80" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent rounded-[3rem]"></div>
            <div className="absolute inset-0 p-10 flex flex-col justify-end text-white text-center items-center">
              <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center mb-6 border border-white/30 group-hover:bg-[#ff007f] transition-colors duration-500">
          ...
          {/* Card 3 : Foire */}
          <div className={`col-span-1 md:col-span-2 lg:col-span-3 bg-gradient-to-br from-[#111] to-[#222] rounded-[3rem] p-10 md:p-16 flex flex-col md:flex-row items-center gap-16 relative overflow-hidden group ${THEME.softShadow}`}>
            {/* Effet abstrait */}
            ...
            <div className="flex-1 w-full relative z-10">
              <div className="grid grid-cols-2 gap-4">
                <img src={DATA.upcomingEvents[1]?.image1 || "/achanger.png"} alt="Foire 1" className="w-full h-56 md:h-72 object-cover rounded-[2rem] transform translate-y-12 border border-white/10 shadow-2xl" />
                <img src={DATA.upcomingEvents[1]?.image2 || "/achanger.png"} alt="Foire 2" className="w-full h-56 md:h-72 object-cover rounded-[2rem] border border-white/10 shadow-2xl" />
              </div>
            </div>
          </div>
        </div>
